  ...
  def render_items
    sections.map do |section|
      content_tag(:li) do
        header = content_tag(
          :div,
          content_tag(:h4, section[:title], class: 'px-2 text-xs font-medium text-gray-400 tracking-wide'),
          class: 'w-full border-b border-gray-200 pb-1 mb-2'
        )
        list = content_tag(:ul, section[:items].map { |item| render_item(item) }.join.html_safe,
                           role: 'list', class: 'mx-4 space-y-1')
        header + list
      end
    end.join.html_safe
  end
